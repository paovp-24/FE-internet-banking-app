...
const Inversion = () => {
  ...
  const { inversiones, postInversion, putInversion, deleteInversion } = useInversion();
  ...
  const handleError = (option, campo) => {
    option === 1 ? Swal.fire("Error de ingreso de inversion", `El campo de ${campo} está vacio`,"error") :
    option === 2 ? Swal.fire("Error de eliminación de inversion", "La inversion no se ha podido eliminar", "error") :
    Swal.fire("Transacción Completa", "La inversion se ha eliminado", "success")
  };
